    ...
    authenticate("auth-jwt") {
        ...
        put("/events/{id}") {
            ...
            val updated = eventService.updateIfOwner(
                ...
                title = req.title,
                description = req.description
            ) ?: return@put call.respond(HttpStatusCode.Forbidden, mapOf("error" to "Not owner or not found"))
        ...
        delete("/events/{id}") {
            ...
            val ok = eventService.deleteIfOwner(eventId = id, ownerId = userId)
            if (!ok) return@delete call.respond(HttpStatusCode.Forbidden, mapOf("error" to "Not owner or not found"))
